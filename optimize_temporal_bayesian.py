...
"""
Parameter optimizer using Bayesian optimization to find optimal temporal tracking parameters.
Tests combinations of max_frames, cluster_distance, and min_detection_ratio on OK/Defect videos.
...
def evaluate_video(video_path, cfg, max_frames, cluster_distance, min_detection_ratio, show_vis=False):
    """
    Evaluate a single video with given temporal parameters.
    Returns duration ratio (frames with >= 8 pills / total frames).
...
def evaluate_parameter_set(max_frames, cluster_distance, min_detection_ratio, cfg, ok_videos, defect_videos, show_vis=False, iteration_num=None):
    """
    Evaluate a parameter set on all videos.
    Returns score (0-1, higher is better):
    - OK videos: maximize duration ratio (time with >= 8 pills)
    - Defect videos: minimize duration ratio (ideally 0%)
    ...
    """
    # Evaluate OK videos (want high duration ratio)
    ok_scores = []
    ...
    for i, video_path in enumerate(ok_videos):
        ...
        ratio = evaluate_video(video_path, cfg, max_frames, cluster_distance, min_detection_ratio, show_vis=vis_enabled)
        ok_scores.append(ratio)
        if iteration_num is not None:
            print(f"      → Duration ratio: {ratio:.2%}")
    
    # Evaluate Defect videos (want low duration ratio, ideally 0%)
    defect_scores = []
    ...
    # OK: higher is better (maximize)
    # Defect: lower is better (minimize, so use 1 - ratio)
    ...
    score = 0.6 * avg_ok + 0.4 * (1.0 - avg_defect)
    
    return score, avg_ok, avg_defect

# ---------- Bayesian Optimization ----------
def optimize_parameters(cfg, ok_videos, defect_videos, n_calls=50, random_state=42, show_vis=False):
    """
    Use Bayesian optimization to find optimal temporal parameters.
    
    Args:
        show_vis: If True, show real-time video processing during optimization
    """
    print(f"\n{'='*60}")
    print(f"Optimizing Temporal Parameters")
    print(f"{'='*60}")
    ...
    # Define search space
    space = [
        Integer(5, 50, name='max_frames'),           # 5-50 frames
        Real(15.0, 50.0, name='cluster_distance'),  # 15-50 pixels
        Real(0.15, 0.5, name='min_detection_ratio') # 0.15-0.5 (15%-50%)
    ]
    ...
    def objective(max_frames, cluster_distance, min_detection_ratio):
        ...
        print(f"    OK avg duration ratio: {ok_ratio:.2%}")
        print(f"    Defect avg duration ratio: {defect_ratio:.2%}")
        if is_best:
    ...
    print(f"  OK videos avg duration ratio: {best_ok_ratio:.2%}")
    print(f"  Defect videos avg duration ratio: {best_defect_ratio:.2%}")
    print(f"{'='*60}\n")
...
# ---------- Main ----------
def main():
    ap = argparse.ArgumentParser(description="Optimize temporal tracking parameters using Bayesian optimization")
    ap.add_argument("--settings", default="", help="JSON settings file (for warp/preprocess/blob)")
    ap.add_argument("--n-calls", type=int, default=50, help="Number of optimization iterations (default: 50)")
    ...
    if not args.auto_run:
        response = input(f"\nRun optimization with {args.n_calls} iterations? (y/n): ")
        if response.lower() != 'y':
            print("Cancelled.")
            return
    
    # Run optimization
    best_params = optimize_parameters(cfg, ok_videos, defect_videos, n_calls=args.n_calls, show_vis=args.show_vis)
